...
$p.addEventListener('click', call('p event'))
$div.addEventListener('click', call('div event'))
$body.addEventListener('click', call('body event'))

// event.target - 최초로 이벤트를 발생시킨 엘리먼트를 가리킨다.
// this (=event.currentTarget) - 현재 이벤트가 발생된 엘리먼트를 가리킨다.

function call2(event){
    console.log(`타깃 엘러먼트 : ${event.target.tagName}
            현재 엘리먼트 : ${this.tagName}`)
}

$body.addEventListener('click',call2);

// 이벤트 위임
const $Menu = document.getElementById('Menu')

const ActionFunctions = {
    save: () => console.log('저장하기'),
    reset: () => console.log('초기화하하기'),
    load: () => console.log('불러오기'),
}
